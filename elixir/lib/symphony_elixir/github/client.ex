defmodule SymphonyElixir.GitHub.Client do
  ...
  def list_pull_requests_for_head(head_ref_name) when is_binary(head_ref_name) do
  ...
  def get_pull_request(pr_number) when is_binary(pr_number) do
  ...
  def list_pull_request_issue_comments(pr_number) when is_binary(pr_number) do
    list_comments(pr_number)
  end

  @spec list_pull_request_reviews(String.t()) :: {:ok, [map()]} | {:error, term()}
  def list_pull_request_reviews(pr_number) when is_binary(pr_number) do
    with {:ok, normalized_pr_number} <- parse_issue_number(pr_number),
  ...
  def list_pull_request_review_comments(pr_number) when is_binary(pr_number) do
    with {:ok, normalized_pr_number} <- parse_issue_number(pr_number),
         {:ok, response} <-
  ...
  def get_pull_request_check_status(pr_number) when is_binary(pr_number) do
    with {:ok, pull_request} <- get_pull_request(pr_number),
